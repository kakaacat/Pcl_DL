...
def download_S3DIS():
    ...
    if not os.path.exists(os.path.join(DATA_DIR, 'indoor3d_sem_seg_hdf5_data')):
        ...
        zipfile = os.path.basename(www)
        os.system('wget %s --no-check-certificate; unzip %s' % (www, zipfile))  # wget：Linux中从指定网址下载
        os.system('mv %s %s' % ('indoor3d_sem_seg_hdf5_data', DATA_DIR))        # mv：Linux中的移动指令
        os.system('rm %s' % (zipfile))                                          # rm：Linux中的删除指令
    if not os.path.exists(os.path.join(DATA_DIR, 'Stanford3dDataset_v1.2_Aligned_Version')):
        if not os.path.exists(os.path.join(DATA_DIR, 'Stanford3dDataset_v1.2_Aligned_Version.zip')):
            ...
            os.system('unzip %s' % (zippath))                                              #解压
            os.system('mv %s %s' % ('Stanford3dDataset_v1.2_Aligned_Version', DATA_DIR))   #移动
            os.system('rm %s' % (zippath))                                                 #移除
...
def prepare_test_data_semseg():
    ...
    if not os.path.exists(os.path.join(DATA_DIR, 'stanford_indoor3d')):
        os.system('python prepare_data/collect_indoor3d_data.py')       # 执行 prepare_data/collect_indoor3d_data.py
    if not os.path.exists(os.path.join(DATA_DIR, 'indoor3d_sem_seg_hdf5_data_test')):
        os.system('python prepare_data/gen_indoor3d_h5.py')             # 执行 prepare_data/gen_indoor3d_h5.py
...
def load_color_partseg():
    colors = []
    labels = []
    f = open("D:/py_Projects/dgcnn.pytorch-master/prepare_data/meta/partseg_colors.txt")
    ...
    for row in range(0, img_size):
        ...
        for column in range(0, img_size):
            ...
            if color_index >= 50:
                cv2.imwrite("D:/py_Projects/dgcnn.pytorch-master/prepare_data/meta/partseg_colors.png", img, [cv2.IMWRITE_PNG_COMPRESSION, 0])
...
def load_color_semseg():
    colors = []
    labels = []
    f = open("D:/py_Projects/dgcnn.pytorch-master/prepare_data/meta/semseg_colors.txt")
    for line in json.load(f):
        colors.append(line['color'])
        labels.append(line['label'])
    ...
    for _ in range(0, img_size):
        ...
        for _ in range(0, img_size):
            ...
            label_index = label_index + 1
            if color_index >= 13:
                cv2.imwrite("D:/py_Projects/dgcnn.pytorch-master/prepare_data/meta/semseg_colors.png", img, [cv2.IMWRITE_PNG_COMPRESSION, 0])
                return np.array(colors)
